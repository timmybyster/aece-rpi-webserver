...
function createNodeVisuals() {
	...
	game.world.setBounds(-2000*zoom, -2000*zoom, xMax*zoom, yCount*zoom);
    updateLines();
    updateNodeVisuals();
}
	...
	//node = elem;
		if (node['type'] === 'ISC-1'){
			...
			var parentId = node['id'];
				$(allNodeData).each(function(index, elem) {
				node1 = elem['Node'];
				if (node1['type'] === 'IB651' && node1['parent_id'] === parentId)
					xCount += 100;
				});
				if (xCount > xMax)
					xMax = xCount;
		...
		if(yCount > xMax + 8){
			zoomInitial = 2000/(yCount + 1600);
		}
		else{
			zoomInitial = 2000/(xMax + 800);
		}
